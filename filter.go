...
// EventNamePredicate creates a predicate that matches ticket listings with an event name similar to the specified event name.
// Similarity is specified as a float, between 0 and 1 (with 0 representing no similarity and 1 representing an exact match).
...
func alwaysPredicate(listing TicketListing) bool { return true }
